...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    /*
        echo'<table>'. '<thead>'. '<tr>'. '<th>'. 'Food Item'. '</th>'. '<th>'. 'Category Name'. '</th>' . '<th>'. 'Description'. '</th>'. '<th>'. 'Food Unit'. '</th>'. '<th>'. 'Count'. '</th>'. '</tr>'.  '</thead>'. '<tr>'. '<td>' .$r['food_item'].   '</td>'. '<td>'  .$r['category_name']. '</td>'. '<td>' .$r['description']. '</td>'. '<td>'. $r['food_unit']. '</td>'. '<td>' . $r['count']. '</td>'.'</tr>'. '</table>'. '<br>'  ;            */
...
<hr>
<div>    
<h4>You can insert an elements on the data base if you click on the next button</h4>    
<form action="index.php" method="GET">
  Display all:
